...
const validateJwtMiddleware = async (c: Context, next: () => Promise<void>) => {
  ...
  try {
    ...
    if (!loginToken.value) {
      ...
      const refreshToken = await kv.get([
        'refresh',
        port,
      ...
      if (refreshToken.value) {
        log.info(
          'requesting user not found in login KV, but found in refresh KV',
          { id: decoded.sub },
        )
        c.header(
          'WWW-Authenticate',
          'Bearer error="invalid_token", error_description="The token claims are outdated or session invalidated."',
        )
        return c.json({
          error: 'token_invalidated',
          message:
            'Your session has been updated or revoked. Please refresh your token.',
          refresh_url: '/auth/refresh',
        }, 401)
      }
      log.info('requesting user not found in KV, they are logged out', {
        id: decoded.sub,
      })
      c.header(
        'WWW-Authenticate',
        'Bearer realm="api", error="invalid_token", error_description="Authentication required. Please log in at /auth/login."',
      )
      return c.json({
        error: 'unauthorized',
